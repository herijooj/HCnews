...
get_city_coords() {
	local city="$1"
	local api_key="${openweathermap_API_KEY:-${OPENWEATHERMAP_API_KEY:-}}"
	...
	local geo_url="https://api.openweathermap.org/geo/1.0/direct?q=${city_query},BR&limit=1&appid=${api_key}"
	local geo_response
...
get_airquality() {
	...
	if [[ -z "$api_key" ]]; then
		echo "❌ API key de clima não configurada."
		return 1
	fi

	local AIR_URL="https://api.openweathermap.org/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${api_key}"
...
_Fonte: OpenWeatherMap · %s_' \
			...
			"$level_desc" \
			"$health_tip" \
			"$(format_pollutant "$pm25")" \
			"$(format_pollutant "$pm10")" \
			"$(format_pollutant "$o3")" \
			"$(format_pollutant "$no2")" \
			"$now"
	fi
