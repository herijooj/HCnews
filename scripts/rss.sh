...
_rss_date_to_unix() {
	...
	*)
	...
	# Calculate timestamp
	local y
	y=$((10#$year)) m=$((10#$mon)) d=$((10#$day))
	local years_since_epoch
	years_since_epoch=$((y - 1970))
	local leap_days
	leap_days=$(((y - 1969) / 4 - (y - 1901) / 100 + (y - 1601) / 400))
	local total_days
	total_days=$((years_since_epoch * 365 + leap_days))
	...
	sec=$((10#$sec))
	local timestamp
	timestamp=$((total_days * 86400 + hour * 3600 + min * 60 + sec))
