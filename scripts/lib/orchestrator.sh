...
hc_orch_start_network_jobs() {
	...
	local ru_location="$3"
	local fetch_ru="${4:-true}"

	start_timing "network_parallel_start"

	local sports_filter_main="${HCNEWS_SPORTS_FILTER_MAIN:-Brasileirão}"
...
hc_orch_fetch_main_data() {
	...
	local month="$4"
	local day="$5"
	local fetch_ru="${6:-true}"

	hc_orch_start_network_jobs "$city" "$saints_verbose" "$ru_location" "$fetch_ru"
	hc_orch_run_local_jobs "$month" "$day"
	hc_orch_collect_network_data "$fetch_ru"
}
